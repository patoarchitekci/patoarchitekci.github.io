...
podcastData.forEach((data, index) => {
  ...
  card.innerHTML = `
        ...
        <div class="mb-0 max-lg:max-w-[550px]">
            <h2 class="font-tomorrow font-semibold lg:text-4xl custom-md:text-custom-3xl sm:text-2xl text-xl !leading-119 text-orange lg:mb-5 mb-2.5 uppercase">${
              data.title
            }</h2>
            <p class="font-normal lg:text-base sm:text-sm text-xs text-white sm:!leading-119 !leading-125 xl:mb-9 lg:mb-7 mb-4 max-w-[805px]">${
              data.description
            }</p>
            <div class="flex flex-wrap items-center gap-1 lg:mb-10 mb-5 ">
                ${data.tags
                  .map(
                    (tag) =>
                      `<span class="font-normal sm:text-sm text-custom-xs sm:!leading-100 leading-120 text-orange">${tag}</span>`
                  )
                  .join("")}
            </div>
            <a href="/podcast.html">
